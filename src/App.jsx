...
export default function App() {
  ...
  function runAnalysis({ goResult = false } = {}) {
    ...
    window.setTimeout(() => {
      try {
        ...
        const base = analyze(__stateForAnalyze, null);
        analyze(__stateForAnalyze, null);
        // [TMP_DEBUG] salary snapshot + computed mismatch check (DELETE AFTER CONFIRM)
        try {
          const sc = __stateForAnalyze?.salaryCurrent;
          const st = __stateForAnalyze?.salaryTarget;
          console.log("[TMP_DEBUG][SAL_INPUTS]", { salaryCurrent: sc, salaryTarget: st, leadershipLevel: __stateForAnalyze?.leadershipLevel });
        ...
        console.log("[TMP_DEBUG][RISKRESULTS_HEAD]", {
          rrType: Array.isArray(base?.decisionPack?.riskResults) ? "array" : typeof base?.decisionPack?.riskResults,
          rrLen: Array.isArray(base?.decisionPack?.riskResults) ? base.decisionPack.riskResults.length : null,
          head: Array.isArray(base?.decisionPack?.riskResults)
            ? base.decisionPack.riskResults.slice(0, 8).map(r => ({ id: r?.id, layer: r?.layer, pr: r?.priority, score: r?.score }))
            : null
        });
        // ✅ 디버그 로그 제거 (운영 안정화)
  ...
  try {
    if (typeof window !== "undefined") {
      window.__DBG_ACTIVE__ = activeAnalysis;
    }
  } catch { }
