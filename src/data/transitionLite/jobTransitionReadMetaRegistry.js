...
function assignTransitionReadMeta(jobIds, meta, industryRelevanceTier) {
  return jobIds.reduce((acc, jobId) => {
    acc[jobId] = { ...meta, industryRelevanceTier };
    return acc;
  }, {});
}
...
export const JOB_TRANSITION_READ_META_REGISTRY = Object.freeze({
  // --- Internal admin / backoffice (minimal) ---
  ...
  // --- Operations / internal (B2C+general sales anomaly fix → primary) ---
  ...assignTransitionReadMeta(
    [
      "JOB_SALES_B2C_SALES",
  ...
  // --- Project / coordination (secondary) ---
  ...assignTransitionReadMeta(
    [
  ...
  // --- Growth / customer-facing (primary) ---
  ...assignTransitionReadMeta(
    [
...
export function getTransitionReadJobMeta(jobId) {
  ...
  return Object.freeze({
    ...
    industryRelevanceTier: meta?.industryRelevanceTier ?? "secondary",
  });
}
