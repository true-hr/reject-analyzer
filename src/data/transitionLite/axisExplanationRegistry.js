...
function buildAxis2DynamicSummary(band, currentValueChainPosition, targetValueChainPosition, currentCoreContext, targetCoreContext) {
  ...
  switch (band) {
    ...
    case "mid_high":
      return `현재 입력에서 보여준 경험이 ${currentFlow} 중심의 맥락에서 쌓였고, 목표 산업도 ${targetFlow} 중심의 비슷한 흐름을 중요하게 봅니다. 고객을 이해하는 방식과 일의 흐름이 꽤 이어지고, 핵심 업무 맥락도 비교적 가깝습니다. 그래서 현재 산업 경험을 목표 산업에도 자연스럽게 연결할 수 있어 높게 평가됐습니다.`;
    case "high":
      return `현재 입력에서 보여준 경험이 ${currentFlow}을 중심으로 쌓였고, 목표 산업도 거의 같은 핵심 업무 흐름을 가집니다. 고객, 시장, 일의 흐름이 직접적으로 이어져 산업이 바뀌어도 설명 방식이 크게 달라지지 않습니다. 그래서 현재 산업 경험을 목표 산업에서도 큰 해석 변경 없이 그대로 강점으로 설명할 수 있어 매우 높게 평가됐습니다.`;
    default:
      return "";
  }
}
...
const AXIS1_ROLE_READING_PROFILES = {
  ...
  JOB_ENGINEERING_DEVELOPMENT_SOFTWARE_DEVELOPMENT: {
    ...
    missingActions: [
      "컴퓨터공학 등 관련 전공은 소프트웨어개발 직무의 기초와 연결되지만, 현재 입력만으로는 실제로 어떤 기능을 직접 구현했는지까지는 충분히 확인되지 않습니다.",
      "API, 데이터베이스, 인증/권한, 화면-서버 연결, 예외 처리처럼 개발 과업을 프로젝트 안에서 다뤄본 근거가 있으면 직무 연결성이 더 선명해집니다.",
      "신입 단계에서는 대규모 운영 경험보다, 작은 기능이라도 요구사항을 이해하고 구현·검증·수정까지 해본 과정이 더 중요한 근거가 됩니다.",
    ],
    followUpActions: [
      "수업, 캡스톤, 개인 프로젝트, 팀 프로젝트에서 직접 구현한 기능이 무엇인지 정리해보세요.",
      "프로젝트를 설명할 때 사용 기술만 적기보다, 어떤 문제를 해결했고 어떤 입력값·출력값·예외 상황을 고려했는지 함께 적어보세요.",
      "GitHub 링크, README, 배포 URL, 테스트 기록, 오류 수정 과정, 팀원과의 역할 분담이 있다면 포트폴리오에 함께 남겨두세요.",
      "백엔드 개발을 희망한다면 API 설계, DB 테이블 구조, 인증/권한 처리, 화면-서버 데이터 흐름 중 본인이 맡은 부분을 짧게 설명할 수 있어야 합니다.",
    ],
  },
  ...
  // Project Management (PM/PO/PL)
  JOB_BUSINESS_PROJECT_MANAGEMENT: {
    jobCoreActions: [
      "PM/PO/PL이라는 직무명 안에서 (1) 일정·범위·납기 통제형 딜리버리 PM/PMO, (2) 프로그램·전략 연계 PM, (3) 기능 정의·요구사항 정리·우선순위 조율·개발 협업 중심의 서비스기획형 PM/PO(프로덕트 매니저, 프로덕트 오너) 중 어떤 갈래의 일을 하게 되는지 JD 기준으로 구분하기",
      "딜리버리 PM/PMO 갈래에서는 프로젝트 목표 정의·계획 수립·일정/리소스/예산 관리·리스크 대응·이해관계자 조율로 실행을 감독",
      "서비스기획형 PM/PO 갈래에서는 사용자 문제 정의, 기능 요구사항 정리, 우선순위 결정, 개발·디자인과의 협업, 배포 후 성과·지표 기반 개선을 반복",
    ],
    majorRelatedActions: [
      "경영학·조직론 전공은 조직 관리, 리더십, 전략적 계획과 직접 연결될 수 있습니다.",
      "산업공학 전공은 프로세스 최적화, 자원 배분, 일정 계획과 연결될 수 있습니다.",
      "수학·통계 전공은 일정 계획, 리스크 분석, 데이터 기반 의사결정과 연결될 수 있습니다.",
      "커뮤니케이션·리더십 역량은 팀 관리, 이해관계자 소통과 일부 연결될 수 있습니다.",
      "UX·서비스기획·디자인 관련 학습·경험은 PM/PO/PL 안에서도 기능 정의·요구사항 정리·사용자 흐름 설계 중심의 서비스기획형 PM/PO(프로덕트 매니저, 프로덕트 오너) 갈래와 연결될 수 있습니다.",
    ],
    missingActions: [
      "전공만으로 실제 프로젝트를 기획하고 리딩한 경험, 또는 제품 기능을 정의하고 출시까지 끌고 간 경험은 확인되지 않습니다.",
      "딜리버리 PM/PMO 갈래라면 프로젝트 계획 수립, 일정 관리, 예산 통제, 위험 요소 대응, 팀 조율, 이해관계자 관리 경험이 별도 근거로 필요합니다.",
      "서비스기획형 PM/PO 갈래라면 사용자 문제 정의, 기능 요구사항 작성, 우선순위 결정, 개발·디자인 협업, 배포 후 지표 기반 개선 경험이 별도 근거로 필요합니다.",
      "자신이 관리한 프로젝트가 정해진 일정·예산·품질 목표를 달성한 성과, 또는 자신이 정의한 기능이 실제 출시되어 의도한 사용 결과로 이어진 증거는 추가 확인이 필요합니다.",
    ],
    followUpActions: [
      "수업이나 캡스톤 프로젝트에서 프로젝트 계획 수립, 일정 관리, 팀 조율, 리스크 관리 또는 기능 정의·요구사항 정리·우선순위 조율을 다룬 경험이 있는지 떠올려보세요.",
      "지원하는 PM/PO/PL 공고의 JD가 일정·범위·납기·리스크 통제 중심인지(딜리버리 PM/PMO), 아니면 기능 정의·요구사항·우선순위·개발 협업 중심인지(서비스기획형 PM/PO)를 먼저 구분한 뒤 이력서에서 강조할 경험을 선택하세요.",
      "인턴십이나 동아리에서 실제 프로젝트를 리딩하거나 부분 담당한 경험, 또는 UX·서비스 설계·기능 개선을 통해 사용자 흐름과 요구사항을 정리한 경험이 있다면 Axis3에서 보강 근거가 됩니다.",
      "단순 계획 수립이나 화면 설계만 보여주기보다 \"이 프로젝트(또는 기능)의 리스크/문제는 무엇이었고, 어떻게 우선순위를 결정해 해결했으며, 최종 결과는 무엇인가\"를 구체적으로 설명할 수 있는 경험이 더 중요합니다.",
      "PM/PO/PL은 일정 통제만, 또는 화면 설계만으로 설명되지 않으며, 실제 JD가 요구하는 갈래에 맞춰 실행·기획 경험이 함께 입증될 때 더 설득력 있게 읽힙니다.",
    ],
  },
...
const NEWGRAD_AXIS2_INDUSTRY_GUIDES = {
  IND_IT_SOFTWARE_PLATFORM_B2B_SAAS: {
    label: "B2B SaaS",
    structure: "B2B SaaS 산업은 개인 사용자가 아니라 기업 고객이 반복적으로 사용하는 소프트웨어를 구독·계약 형태로 제공하는 구조입니다. 제품 기능 자체뿐 아니라 도입, 온보딩, 사용 지속, 보안·권한 관리, 고객사의 업무 프로세스와의 연결이 중요하며, 비용을 승인하는 의사결정자와 실제로 기능을 쓰는 사용자가 다를 수 있어 구매 과정과 사용 흐름을 함께 봐야 합니다.",
    ...
    gap: "현재 입력에는 직접적인 SaaS 경험이 없어 산업 맥락을 다뤘다는 근거가 드러나지 않습니다. 고객사의 업무 문제, 구독 수익 구조, 온보딩 프로세스, 기능 활용과 활성화, 이탈 가능성, 영업·CS 접점을 별도로 분석해 산업 이해도를 보완해야 합니다.",
    lift: "직접 경험이 없다면 협업툴, CRM, 프로젝트 관리툴 같은 B2B SaaS를 하나 정해 고객사 유형, 도입 이유, 의사결정자와 실사용자의 차이, 온보딩 단계, 핵심 기능 사용 장면, 활성화 지표, 이탈 위험 요소를 분석해보는 방식으로 산업 이해도를 보완할 수 있습니다.",
  },
  ...
  IND_MANUFACTURING_MACHINERY_INDUSTRIAL_EQUIPMENT: {
    label: "제조업 (기계·산업장비)",
    structure: "제조업·기계·산업장비 산업은 부품을 설계·조달해 기계를 생산하고, 품질·성능을 보증하며, 고객에게 설치·유지·AS까지 제공하는 구조입니다. 원가 효율성, 생산 공정, 제품 신뢰성, 공급망 안정성, 납기 준수가 산업 이해의 핵심이며, 생산, 설비, 품질, 구매, 연구개발, 영업/AS 등 여러 기능이 연결되어 움직입니다. 기계·산업장비 분야는 B2B 고객, 기술 사양, 현장 조건, 유지보수 맥락뿐 아니라 생산·설비·품질·구매·연구개발 등 제조 조직의 직무 구조도 함께 작동합니다.",
    strongMajorFit: "현재 입력만 보면 제조 산업과의 기초 연결은 어느 정도 있습니다. 기계공학 전공은 제품 설계, 기술 사양, 성능 기준, 공정 이해 같은 기계 제조의 기술 기반과 연결될 수 있고, 회계학 전공은 원가 구조, 재무 성과, 비용 효율성 같은 제조 경영의 재무 기초와 연결될 수 있습니다.",
    weakMajorFit: "현재 입력만 보면 제조 산업을 구체적으로 이해했다는 근거는 아직 제한적입니다. 이 산업에서는 기술이나 숫자만으로는 부족하고, 실제로 안정적인 생산을 위해 공정 개선, 품질 관리, 공급망 조율, 현장 운영을 어떻게 풀어나가는지가 중요합니다.",
    gap: "다만 현재 입력만으로는 실제 생산 라인 운영, 공정 최적화, 품질 이슈 해결, 원가·납기 관리, 부품 공급망 조율, 제조 데이터 분석, 기술 표준과 규제 대응, 현장 인력 구성과 숙련도 관리까지 직접 다뤄봤는지는 드러나지 않습니다. 지원 직무와 제조 운영 흐름이 어떻게 연결되는지 별도로 정리해야 산업 이해도가 더 선명해집니다.",
    lift: "제조 현장 견학, 재무제표 분석, 비용 분석 경험은 제조업의 현장 운영 구조나 비용 흐름을 이해하는 초기 근거로 연결될 수 있습니다. 특히 생산량, 원가, 재고, 구매 단가, 설비·공정 비용, 품질 이슈뿐 아니라 현장 직무, 기술 인력, 안전·품질 문화가 성과에 어떤 영향을 주는지까지 정리하면 지원 근거가 더 구체화됩니다.",
  },
  IND_MANUFACTURING_SEMICONDUCTOR_DISPLAY: {
    label: "반도체 / 디스플레이",
    structure: "반도체·디스플레이 산업은 설계(팹리스/IDM) → 전공정(증착·식각·포토) → 후공정(패키징·테스트) → 모듈/패널 조립으로 이어지는 공정 체계 위에, 소재·장비·부품 공급망과 Fab 운영이 맞물리는 구조입니다. 수율, 공정 불량, 품질 기준, 납기·원가, 고객사(IDM·파운드리·팹리스·패널메이커) 요구사항 대응이 산업 이해의 핵심이며, 공정·설비·품질·구매·영업·기술지원 등 여러 기능이 연결되어 움직입니다.",
    strongMajorFit: "현재 입력만 보면 반도체·디스플레이 산업과의 기초 연결은 어느 정도 있습니다. 전자·전기 전공은 반도체 소자 원리, 회로 이론, 전력·설비 기초와 연결될 수 있고, 전기기사 자격증은 전력·설비 분야의 기초 배경을 보여줍니다. 이 기초 배경은 기술 사양을 이해하고 현장 언어에 접근하는 출발점이 될 수 있습니다.",
    weakMajorFit: "현재 입력만 보면 반도체·디스플레이 산업을 구체적으로 이해했다는 근거는 아직 제한적입니다. 이 산업에서는 소자 이론보다 실제 공정 흐름(CVD·식각·포토·패키징), 수율 분석, Fab 운영 구조, 고객사 유형별 요구사항 차이가 중요합니다.",
    gap: "다만 현재 입력만으로는 실제 공정 흐름, 수율 관리, 품질·불량 분석, 고객사(IDM·파운드리·팹리스·패널메이커) 구조 이해, 소재·장비 공급망 조율, 기술 영업 또는 기술 지원 맥락에서의 고객 대응을 직접 다뤄봤는지는 드러나지 않습니다. 전공 이론이 반도체·디스플레이 실무와 어떻게 연결되는지 별도로 정리해야 산업 이해도가 더 선명해집니다.",
    lift: "공정 기술 자료 탐색, 반도체·디스플레이 기업 제품 라인업과 고객사 구조 파악, 수율·불량·원가 흐름에 대한 기초 학습은 산업 이해를 보강하는 출발점이 될 수 있습니다. 전자·전기 전공 수업이나 실험에서 다룬 소자, 회로, 전력 개념이 반도체 공정의 어떤 단계와 연결되는지 정리하면 지원 근거가 더 구체화됩니다.",
  },
};
