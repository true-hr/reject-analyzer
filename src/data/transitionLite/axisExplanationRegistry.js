...
function buildAxis2DynamicSummary(band, currentValueChainPosition, targetValueChainPosition, currentCoreContext, targetCoreContext) {
  ...
  switch (band) {
    case "very_low":
      return `현재 입력의 산업 배경과 목표 산업은 고객을 이해하는 방식, 시장을 보는 기준, 실제 일의 흐름이 모두 다르게 읽힙니다. 목표 산업에서는 ${targetFlow}을를 중요하게 보므로, 이런 산업 맥락을 별도로 학습하고 설명할 수 있어야 연결성이 생깁니다. 그래서 현재 입력만으로는 매우 낮게 평가됐습니다.`;
    case "low":
