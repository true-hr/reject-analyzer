...
const NEWGRAD_AXIS2_JOB_INDUSTRY_SPEC = Object.freeze({
  ...
  digital_healthcare: Object.freeze({

    SOFTWARE_DEVELOPMENT: Object.freeze({
      // Row 1 — background
      backgroundWeakEvidenceText:
        "컴퓨터공학·소프트웨어 관련 배경이 있더라도, 디지털 헬스케어에서 다루는 건강·의료 데이터의 민감성, 사용자 신뢰, 의료진·환자 사용 맥락이 기능 구현에 어떻게 반영돼야 하는지는 아직 명확하지 않습니다.",
      backgroundModerateEvidenceText:
        "개발 학습·프로젝트 경험이 있어 기능 구현의 기초는 있으나, 의료 데이터의 민감성, 인증·권한 분리, EMR/HIS 같은 외부 시스템과의 API 연동, 의료진·환자의 사용 흐름까지 함께 고려해본 경험은 더 확인이 필요합니다.",
      backgroundStrongEvidenceText:
        "개발 배경과 함께 건강·의료 데이터, 개인정보보호, 의료진·환자 사용 흐름을 고려한 프로젝트 경험이 보이면, 디지털 헬스케어 개발 맥락으로 비교적 자연스럽게 연결될 수 있습니다.",
      backgroundLimitText:
        "다만 신입 단계에서는 (1) 건강·의료 데이터가 일반 데이터와 어떻게 다른지(민감성·정확성·신뢰), (2) 인증·권한 분리와 개인정보보호가 기능 설계에 어떻게 반영되는지, (3) 의료진·환자라는 두 종류의 사용자가 같은 화면을 보는 방식 차이, (4) 외부 시스템(병원 시스템·표준 의료 데이터 포맷)과의 연동 가능성까지 한 번이라도 고려해본 흔적이면 충분합니다. EMR/HIS 연동 실무, FHIR/HL7 같은 표준은 알면 좋지만 신입에게 필수처럼 단정하지 않습니다.",

      // Row 2 — work context
      workContextWeakEvidenceText:
        "디지털 헬스케어 개발 실무 환경을 직접 경험했다고 볼 만한 근거는 아직 명확하지 않습니다.",
      workContextModerateEvidenceText:
        "웹·앱·API·DB 같은 기본 구현 경험은 일부 감지되나, 그 기능이 건강·의료 데이터를 다루거나 의료진·환자의 사용 흐름과 맞물리거나 권한·보안 조건을 고려한 맥락까지는 확인이 필요합니다.",
      workContextStrongEvidenceText:
        "건강·의료 데이터 기반 기능 구현, 개인정보·권한 고려, 의료진·환자 사용 흐름, 병원·기관 시스템 연동을 함께 다룬 경험이 보이면 디지털 헬스케어 개발 맥락의 실무 조건을 이해할 가능성이 비교적 높게 읽힙니다.",
      workContextLimitText:
        "신입 지원자라면 직접 운영 경험이 없더라도 (1) 작은 기능이라도 건강·의료 데이터의 민감성을 고려해 입력값·예외 상황을 다뤄본 흔적, (2) 로그인·권한 분리·민감정보 마스킹 같은 보안 기본 요소를 직접 적용해본 경험, (3) 의료진·환자처럼 신뢰 기준이 다른 사용자 시나리오를 분리해 화면·플로우를 정리해본 경험, (4) 외부 API·표준 데이터 포맷과 연동을 시도해본 경험 같은 준비 근거를 보완하세요. 대규모 트래픽 운영·서버 아키텍처 설계·장애 대응·성능 최적화 주도 같은 시니어형 경험을 신입에게 요구하지 않습니다.",

      // Row 3 — repeatability
      weakEvidenceText:
        "디지털 헬스케어 맥락에서 개발을 반복적으로 다뤄본 흔적은 아직 제한적으로 읽힙니다.",
      moderateEvidenceText:
        "개발 경험은 반복되지만, 그 반복이 건강·의료 데이터, 사용자 신뢰, 의료 도메인 시나리오와 연결되는 방향으로 누적됐는지는 추가 확인이 필요합니다.",
      strongEvidenceText:
        "건강·의료 데이터를 다루는 기능 구현, 개인정보·권한 고려, 의료진·환자 사용 흐름, 외부 의료 시스템 연동 같은 맥락이 여러 차례 반복적으로 보이면 디지털 헬스케어 개발 맥락에 누적된 관심으로 읽힙니다.",
      repeatabilityLimitText:
        "한 단계 더 강화하려면 (1) 일반 웹·앱 개발 경험과 건강·의료 데이터를 다루는 개발의 차이(민감성·정확성·신뢰)를 정리해보기, (2) 의료진·환자처럼 다른 사용자 그룹을 같은 시스템 안에서 어떻게 분리해 다룰지에 대한 화면·권한 설계 연습, (3) 인증·권한·민감정보 마스킹을 작은 프로젝트에 적용해본 흔적, (4) EMR/HIS·FHIR/HL7 같은 표준 의료 데이터 포맷을 한 번이라도 살펴본 경험 같은 각도를 정리하면 좋습니다. 신입 단계에서 모든 항목을 갖춰야 한다는 의미는 아닙니다.",
    }),
  ...
  food_consumer_goods: Object.freeze({

    RECRUITING: Object.freeze({
      // Row 1 — background
      backgroundWeakEvidenceText:
        "경제학 등 사회과학 전공은 자료 비교, 기준 정리, 노동시장 이해와 연결될 수 있지만, 현재 입력만으로는 식품/소비재 제조 기업의 채용 구조나 직무별 인재 요건을 이해한 배경까지는 명확하지 않습니다.",
      backgroundModerateEvidenceText:
        "전공 배경과 기업·시장 이해가 채용 직무의 지원자 정보 비교, 직무 요건 정리, 인력 수요 해석의 기초가 될 수 있습니다. 다만 식품/소비재 제조 기업에서 생산·품질·영업·마케팅·물류 직무별 요구 역량이 어떻게 달라지는지까지 정리한 근거는 더 필요합니다.",
      backgroundStrongEvidenceText:
        "전공 배경과 함께 식품/소비재 제조 기업의 직무 구조, 인재상, 채용공고, 조직 운영 방식을 조사한 근거가 보여 채용 직무의 산업 이해 기반으로 비교적 강하게 읽힙니다.",
      backgroundLimitText:
        "다만 산업 이해를 더 설득하려면 (1) 식품/소비재 제조 기업의 채용공고 3~5개 비교, (2) 생산·품질·영업·마케팅·물류 직무별 요구 역량 정리, (3) 신제품 출시·성수기 수요·품질 이슈가 인력 수요와 채용 기준에 어떤 영향을 주는지 조사한 근거를 보완하는 것이 좋습니다.",

      // Row 2 — work context
      workContextWeakEvidenceText:
        "식품/소비재 제조 기업의 채용 업무 환경을 직접 이해했다고 볼 만한 근거는 아직 명확하지 않습니다.",
      workContextModerateEvidenceText:
        "채용 직무에 대한 관심은 보이나, 식품/소비재 제조 기업에서 현업 부서가 어떤 인재를 필요로 하고 채용 담당자가 그 기준을 어떻게 정리해야 하는지까지는 추가 확인이 필요합니다.",
      workContextStrongEvidenceText:
        "식품/소비재 제조 기업의 직무별 채용공고 분석, 현업 요구역량 정리, 후보자 기준표 작성, 채용 프로세스 조사 경험이 보여 채용 직무의 실제 업무 환경 이해로 강하게 읽힙니다.",
      workContextLimitText:
        "신입 지원자라면 직접 채용 운영 경험이 없더라도 (1) 생산·품질·영업·마케팅·물류 직무별 채용공고 비교, (2) 직무별 필수 역량과 우대사항 정리, (3) 식품/소비재 제조 기업의 채용 절차와 평가 기준 조사, (4) 성수기·신제품·품질 이슈가 인력 수요에 미치는 영향 정리 같은 준비 근거를 보완하세요.",

      // Row 3 — repeatability
      weakEvidenceText:
        "식품/소비재 제조 산업과 채용 직무를 함께 이해했다는 반복 신호는 아직 제한적으로 읽힙니다.",
      moderateEvidenceText:
        "식품/소비재 제조 기업의 채용공고나 직무별 인재상을 일부 접한 흔적이 있어 산업 이해의 기초로 볼 수 있습니다. 다만 여러 직무군을 비교하거나 인력 수요 변화까지 정리한 흐름은 더 필요합니다.",
      strongEvidenceText:
        "식품/소비재 제조 기업의 여러 직무군 채용공고를 비교하고, 생산·품질·영업·마케팅·물류 등 직무별 인재 요건과 채용 기준을 반복적으로 정리한 근거가 보여 산업 이해도가 비교적 높게 읽힙니다.",
      repeatabilityLimitText:
        "한 단계 더 강화하려면 (1) 식품/소비재 제조 기업 3곳의 채용공고 비교표, (2) 생산·품질·영업·마케팅·물류 직무별 요구역량 정리, (3) 신제품 출시·성수기 수요·품질 이슈가 인력 운영과 채용 기준에 미치는 영향, (4) 지원 회사의 최근 제품군과 조직 구조를 채용 직무 관점으로 정리해보세요.",
    }),
