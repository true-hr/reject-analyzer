...
const _AXIS3_ROW_REWRITE_GUIDANCE = Object.freeze({
  outcome_level: Object.freeze({
    ...
    instruction: "currentRows의 rawLimitRef, rawMissingHint, rowRole, guidance는 모두 참고자료다. 이 중 어느 것도 missingEvidenceLabel이나 jobIndustryCriterion에 그대로 복사하면 안 된다. missingEvidenceLabel은 반드시 목표 직무×산업 언어로 새 문장을 작성하라. 산업별 수행 기준 필수 포함: 커머스 서비스기획→상품 탐색/구매 전환/고객 행동 흐름, 금융 서비스기획→금융 정보 이해도/신뢰 형성/리스크 고지, 제조 데이터분석→공정 데이터/불량률/원인 분석/개선 의사결정.",
...
function _buildAxis3RowRewriteGuidance(rowKey) {
  return _AXIS3_ROW_REWRITE_GUIDANCE[rowKey] ?? null;
}
...
function extractResponsibilityScopeAxis(axis, resultVm) {
  ...
  const outcomeRows = toArr(axis.comparisonBlock?.rows)
    .filter((row) => toStr(row?.rowKey) === "outcome_level")
    .slice(0, 1)
    .map((row) => ({
      rowKey: toStr(row?.rowKey),
      label: toStr(row?.label || row?.itemLabel),
      ...
      verdictText: truncateText(row?.verdictText),
      evidenceText: truncateText(row?.evidenceText),
      rawLimitRef: truncateText(row?.limitText),
      currentValue: truncateText(row?.currentValue, 80),
      positiveEvidenceLabels: toArr(row?.positiveEvidenceLabels).slice(0, 2).map((v) => truncateText(v, 100)),
      rawMissingHint: toArr(row?.missingEvidenceLabels).slice(0, 2).map((v) => truncateText(v, 100)),
      rowRewriteGuidance: _buildAxis3RowRewriteGuidance(toStr(row?.rowKey)),
    }));
