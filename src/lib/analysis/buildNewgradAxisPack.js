...
const _JOB_MAJOR_ROLE_MAP = {
  ...
  MARKETING:                        { direct: ["marketing"], adjacent: ["planning", "data_analytics", "design", "content_sns_media_ops", "pr_public_relations_comms"] },
  CUSTOMER_OPERATIONS:              { direct: ["operations_customer_support", "operations_support", "field_service_retail_ops"], adjacent: ["sales_business_development", "planning"] },
  HR_ORGANIZATION:                  { direct: ["hr_management_support"], adjacent: ["planning", "operations_customer_support", "operations_support"] },
  FINANCE_ACCOUNTING:               { direct: ["planning", "hr_management_support", "finance_accounting_support"], adjacent: ["data_analytics"] },
  PROCUREMENT_SCM:                  { direct: ["operations_customer_support", "operations_support", "procurement_scm_logistics", "merchandise_ecommerce_ops"], adjacent: ["planning", "sales_business_development"] },
  MANUFACTURING_QUALITY_PRODUCTION: { direct: ["operations_support", "operations_customer_support", "manufacturing_quality_production_ops"], adjacent: ["planning"] },
  ENGINEERING_DEVELOPMENT:          { direct: ["backend_development", "frontend_development", "development"], adjacent: ["data_analytics", "research_lab_rd_support", "manufacturing_quality_production_ops"] },
  IT_DATA_DIGITAL:                  { direct: ["data_analytics", "development", "backend_development", "frontend_development", "data_labeling_ai_qa_ops"], adjacent: ["planning", "design"] },
  DESIGN:                           { direct: ["design"], adjacent: ["frontend_development", "marketing", "content_sns_media_ops"] },
  RESEARCH_PROFESSIONAL:            { direct: ["data_analytics", "research_lab_rd_support"], adjacent: ["planning"] },
  EDUCATION_COUNSELING_COACHING:    { direct: ["operations_customer_support", "operations_support", "education_counseling_coaching_ops"], adjacent: ["planning", "hr_management_support"] },
  PUBLIC_ADMINISTRATION_SUPPORT:    { direct: ["planning", "operations_customer_support", "operations_support", "legal_compliance_public_admin"], adjacent: ["hr_management_support"] },
};
...
function buildAxis2ComparisonBlock(signals = {}) {
  ...
  const _bgArchetype = getIndustryArchetype(targetIndustryLabel);
  ...
  const _neutralWorkContextLimitText = targetIndustryLabel
    ? `현재 입력만으로는 ${targetIndustryLabel} 산업 안에서 해당 직무가 어떤 업무 흐름, 운영 개선 대상, 협업 구조와 맞닿는지 직접 근거가 부족합니다.`
    : "현재 입력만으로는 이 산업 안에서 해당 직무가 어떤 업무 흐름과 산출물에 연결되는지 직접 근거가 부족합니다.";
  const _neutralRepeatabilityLimitText = "이 산업에서 해당 직무와 연결되는 반복 경험은 아직 제한적으로 보입니다. 산업 일반 문서 처리 경험보다, 목표 직무가 개선할 수 있는 운영 흐름이나 산출물 단위를 더 구체화할 필요가 있습니다.";
...
export function buildNewgradAxisPack(input = {}) {
  ...
  const _jobFit         = makeAxis("전공과 직무의 연결성", scoreJobFit(normalized), {
    ...
    targetJobCategory:     _jobFitMajorPrior.targetMajorCategory ?? "",
    targetSubcategory:     _jobFitMajorPrior.targetSubcategory ?? "",
    majorMatchLevel:        _jobFitMajorMatchLevel,
