...
export default function RadarSelfCheck({ selfCheck }) {
  const data = useMemo(() => {
    const sc = selfCheck || {};
    const points = [
      { key: "coreFit", label: "??, v: sc.coreFit ?? 3 },
      { key: "proofStrength", label: "利앷굅", v: sc.proofStrength ?? 3 },
      { key: "roleClarity", label: "紐낇솗", v: sc.roleClarity ?? 3 },
      { key: "storyConsistency", label: "?ㅽ넗由?, v: sc.storyConsistency ?? 3 },
      { key: "riskSignals", label: "由ъ뒪??, v: sc.riskSignals ?? 3 },
  ...
  return (
    <div className="rounded-xl border bg-background/60 p-3">
      <div className="text-xs text-muted-foreground mb-2">?먭?吏꾨떒 ?덉씠??(蹂댁“ ?좏샇)</div>
