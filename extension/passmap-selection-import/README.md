...
ChatGPT 화면에서 업무 대화를 PASSMAP AI Inbox 후보로 보내는 Chrome 확장입니다. 보낸 내용은 바로 이력서 재료로 확정되지 않으며, AI Inbox에서 맞는 내용만 확인해 확정합니다. Claude / Gemini는 현재 자동 캡처 품질 점검 중이므로 필요한 부분을 선택해 보내는 fallback을 권장합니다.
...
popup의 **AI Inbox에 후보로 보내기** 버튼은 저장 토큰이 준비된 경우 `/api/save-analysis-run?action=browser_extension_save_experience`로 구조화 후보를 직접 보냅니다. 토큰이 없으면 요청을 보내지 않고 연결 필요 안내를 표시합니다.

직접 저장이 성공하면 popup에 **Inbox에서 확인하기** 버튼이 표시됩니다. 버튼은 API 응답의 `inboxUrl`을 우선 열고, 없으면 PASSMAP AI Inbox URL로 이동합니다. 성공 메시지는 사용자가 “후보 저장”과 “이력서 재료 확정”을 구분하도록 안내합니다.

현재 직접 AI Inbox 저장은 ChatGPT 대화 화면에서만 지원합니다. Claude/Gemini는 자동 읽기 품질을 더 확인한 뒤 활성화할 예정이므로, 필요한 대화 부분을 드래그한 뒤 **선택한 부분만 저장**을 사용해 주세요.

직접 저장 payload는 `sourcePlatform`, `sourceUrl`, `sourceTitle`, `captureMode`, `captureQuality`, `messageCount`와 서버 분석용 bounded `messages`만 보냅니다. `rawText`, `fullTranscript` 같은 전체 원문 필드는 보내지 않으며, 서버도 전체 원문을 영구 저장하지 않습니다.
...
4. 이후 **AI Inbox에 후보로 보내기** 버튼이 저장 token으로 직접 저장 API를 호출합니다.
