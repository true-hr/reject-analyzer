...
| 항목 | 값 |
|---|---|
| 결과 파일명 | `passmap-selection-import-<version>.zip` (예: `passmap-selection-import-0.1.1.zip`) |
...
background.js
content-passmap.js
icons/icon16.png       (디자인 확정 후 추가)
icons/icon32.png       (디자인 확정 후 추가)
icons/icon48.png       (디자인 확정 후 추가)
icons/icon128.png      (디자인 확정 후 추가)
```

문서(`.md`) 파일은 zip에 포함하지 않습니다. 스토어 listing은 별도 입력 양식으로 처리됩니다.

`manifest.json` 에 `icons` 항목이 추가되면 위 4개 PNG는 반드시 같이 포함되어야 합니다. 아이콘이 아직 없다면 manifest의 `icons` 항목도 추가하지 마세요. 누락된 아이콘 경로는 manifest 검증에서 실패합니다.
...
2. 확장 파일 복사 (포함 대상만 명시적으로 지정)
   ...
   # 아이콘 디렉터리가 존재할 때만:
   if (Test-Path "$src\icons") { Copy-Item "$src\icons" $work -Recurse }
   ```
...
3. 잘못 포함된 파일이 없는지 확인
   ...
   ```
   기대 결과: `manifest.json`, `background.js`, `content-passmap.js`, (아이콘 4종).
...
# 아이콘 디렉터리가 존재할 때만
[ -d "$src/icons" ] && cp -R "$src/icons" "$work/"
