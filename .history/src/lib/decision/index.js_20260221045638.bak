...
export function buildDecisionPack({ state, ai, structural } = {}) {
  ...
  console.log("[DBG_DECISION_PACK_GATE]", { gateBoostValue, hasGateBoost: gateBoostValue > 0, topGate: (Array.isArray(riskResults) ? riskResults.find(r => r?.layer === "gate") : null) });
