...
outcome_level row의 rowRewriteGuidance.rawIndustryCopyPolicy가 "forbidden"이다. responsibilityScope.currentRows[0]의 limitText, missingEvidenceLabels, rowRole, guidance는 모두 참고자료다. 이 문장들을 missingEvidenceLabel이나 jobIndustryCriterion에 그대로 복사하거나 거의 그대로 사용하면 안 된다. 반드시 목표 직무×산업 기준으로 새 문장을 작성하라.
...
currentRows.limitText, currentRows.missingEvidenceLabels, rowRole, guidance를 복사하거나 요약하는 필드가 아니다.
...
반드시 [산업별 수행 기준 키워드] 1개 이상과 [역할/산출물/지표/의사결정 연결] 1개 이상을 포함한다:
  ...
  - 금융 서비스기획: 금융 정보 이해도, 신뢰 형성, 리스크 고지, 오해 방지
...
- missingEvidenceLabel이 currentRows[0].limitText와 거의 같음 → fallbackReason: "copied_current_row_text"
- missingEvidenceLabel이 currentRows[0].missingEvidenceLabels[0]와 거의 같음 → fallbackReason: "copied_current_row_text"
...
- "단순 활동명 나열보다, 본인이 맡은 역할과 만든 산출물이 더 선명해야 한다." (limitText 복사)
- "단순 활동명 나열보다, 분석한 지표와 의사결정 연결이 더 선명해야 합니다." (limitText 소폭 변환)
- "성과 경험이 부족합니다." (일반 문장)
