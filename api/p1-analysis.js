...
outcome_level row의 rowRewriteGuidance.rawIndustryCopyPolicy가 "forbidden"이다. responsibilityScope.currentRows[0]의 rawLimitRef(원문 한계 문장), rawMissingHint(원문 누락 힌트), rowRole, guidance는 모두 참고-전용 입력이다. 이 문장들을 missingEvidenceLabel이나 jobIndustryCriterion에 그대로 복사하거나 거의 그대로 사용하면 안 된다. 반드시 목표 직무×산업 기준으로 새 문장을 작성하라.
...
currentRows.rawLimitRef, currentRows.rawMissingHint, rowRole, guidance를 복사하거나 요약하는 필드가 아니다.
...
- missingEvidenceLabel이 currentRows[0].rawMissingHint[0]와 거의 같음 → fallbackReason: "copied_current_row_text"
...
14. row.rowRewriteGuidance.rawIndustryCopyPolicy가 "forbidden"이면, 해당 row의 rawLimitRef와 rawMissingHint는 산업 공통 참고-전용 입력이다. 이 문장을 missingEvidenceLabel에 그대로 복사하거나 거의 그대로 사용하면 안 된다.
