...
export default function App() {
  ...
  const canAnalyze = useMemo(() => {
    ...
    const s = (() => {
      ...
      try { window.__DBG_LAST_STATE__ = { jdLen: String(merged.jd || "").length, resumeLen: String(merged.resume || "").length, jdTextLen: String(merged.jdText || "").length, resumeTextLen: String(merged.resumeText || "").length }; } catch { }
      return merged;
    })();
        ...
        <CardHeader className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {typeof activeAiMeta?.usedAI === "boolean" ? (
              <Badge className="rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-700">
