-- ============================================================
-- Protected DB migration candidate: 20260531_reminder_rules.sql
-- ============================================================
-- DO NOT APPLY without explicit user approval for Protected DB migration.
-- Do not run this file in Supabase SQL Editor, Supabase CLI, or any remote
-- database until the Protected DB apply step is explicitly authorized.
--
-- Goal:
...
--
-- Rollout risk:
--   Existing send-weekly-experience-recall-push and the future rule-based
--   scheduler can send duplicate weekly reminders if both cron jobs are active.
--   중복 발송 방지를 위해 기존 weekly cron과 신규 scheduler cron을 동시에
--   켜지 않는다.
--   Keep the new scheduler cron disabled until the old weekly cron is off.
--
-- Compatibility:
--   - Does not drop or alter reminder_preferences.
--   - Does not drop or alter reminder_deliveries.
--   - Do not delete reminder_preferences or reminder_deliveries during rollout.
...
comment on table public.reminder_rules is
  'User-owned reminder rules for experience recall scheduling. Protected DB migration candidate; do not apply without approval.';
...
-- PRE-APPLY CHECKLIST
-- ============================================================
-- 1. Get explicit user approval for Protected DB migration apply.
-- 2. Confirm this file is being applied to the intended Supabase project.
-- 3. Confirm public.set_updated_at() exists in the target DB.
-- 4. Confirm reminder_preferences, reminder_deliveries, push_subscriptions,
--    and auth.users exist.
-- 5. Confirm Edge Function scheduler will use service role for delivery writes.
-- 6. Confirm the new scheduler cron is NOT active while the existing weekly
--    cron is active. Running both can cause duplicate weekly reminders
--    / 중복 발송.
-- 7. Confirm UI read order during transition:
--      reminder_rules first; fallback to reminder_preferences if no rules exist.
-- 8. Confirm existing send-weekly-experience-recall-push remains enabled until
--    the new scheduler is deployed and cron is explicitly switched.
-- 9. Confirm no production deploy, cron change, or Edge Function deploy is
--    bundled into the DB apply step.
--
-- ROLLBACK / SAFETY NOTES
-- ============================================================
-- This migration only adds new tables and backfills from existing preference
-- rows. Existing reminder_preferences and reminder_deliveries are untouched.
--
-- Backfill is idempotent: re-running it should not create another identical
-- legacy-derived weekly rule for the same user/time/day/timezone.
--
-- If rollback is required before any new scheduler/UI depends on these tables,
-- the new tables can be dropped in this order:
--   drop table if exists public.reminder_rule_deliveries;
--   drop table if exists public.reminder_rules;
--
-- Do not drop old tables as part of rollback:
--   public.reminder_preferences
--   public.reminder_deliveries
--
-- If the new scheduler has already written deliveries, export or inspect
-- reminder_rule_deliveries before dropping it.
-- ============================================================
