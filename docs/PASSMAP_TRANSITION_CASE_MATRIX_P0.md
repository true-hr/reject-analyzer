...
| 적용 axis | `jobStructure` (lead, scoreReason, criteria), `responsibilityScope` (lead, liftOrLimit) |
| 남은 항목 | boundary fixture (TR-BOUNDARY-CS-TO-SERVICE-001), co-fire fixture (TR-COFIRE-CS-TO-SERVICE-001) |
...
| status | `IMPLEMENTED_FIRST_PASS` |
| 구현 파일 | `src/lib/analysis/careerTransitionCaseOverlays.js` |
| 적용 axis | `jobStructure` (lead, scoreReason, criteria), `industryContext` (lead, liftOrLimit) |
| 남은 항목 | responsibilityScope/roleCharacter already high(100) — 미적용 (과잉 설명 방지) |
...
| 구현 파일 | `src/lib/analysis/careerTransitionCaseOverlays.js` |
| 적용 axis | `jobStructure` (lead, scoreReason, criteria), `responsibilityScope` (lead, liftOrLimit) |
| 구현 범위 | `JOB_MARKETING_PERFORMANCE_MARKETING` → `JOB_BUSINESS_SERVICE_PLANNING` 단일 pair (F-2C) |
