...
| NG-BOUNDARY-MAJOR-001 | Invariant | 경영학 전공 + 서비스기획 → WEAK_MAJOR 오발화 방지 | P0 | FIXTURED | 경영학은 서비스기획 major prior="direct"(3) → WEAK_MAJOR_STRONG_RELEVANT_PROJECT 미발화. jobStructure.lead에 "전공 연결성 제한적" 계열 문구 미노출 계약. shouldNotMention 간접 검증. | 2026-04-30 | - | PASSING (runner 12 PASS, Round D-1 P3) |
| NG-BOUNDARY-MAJOR-003 | Invariant | 경제학 전공 + 서비스기획 → WEAK_MAJOR 오발화 방지 | P0 | FIXTURED | 경제학은 서비스기획 major prior="adjacent"(2) → WEAK_MAJOR_STRONG_RELEVANT_PROJECT 미발화. jobStructure.lead에 "전공 연결성 제한적" 계열 문구 미노출 계약. shouldNotMention 간접 검증. E-1 경영학 direct와 대칭 boundary. | 2026-04-30 | - | PASSING (runner 12 PASS, Round D-1 P3) |
...
| NG-COVERAGE-DEV-001 | Coverage | 비전공 + 데이터분석 + 프로젝트/실무/자격증 전무 | P1 | FIXTURED | NO_EVIDENCE_NON_MAJOR_FOR_DEV_DATA 발화 계약. responsibilityScope.lead/scoreReason/criteria/liftOrLimit(4슬롯). E-3 coverage gap 조사 후 추가. 기존 NG-JOB-DATA-001(certs 있음), NG-JOB-DEV-002(projects 있음)와 비중복. | 2026-04-30 | - | PASSING (runner 12 PASS, Round E-5) |
| NG-COVERAGE-DEV-002 | Coverage | 비전공 + 데이터분석 + 무경험 + 자기보고 강점 → co-fire | P1 | FIXTURED | NO_EVIDENCE(responsibilityScope) + SELF_REPORT(roleCharacter) co-fire 계약. 두 pattern 동시 발화, 서로 다른 axis, slot conflict 없음 고정. NG-COVERAGE-DEV-001(strengths 없음)과 대칭. | 2026-04-30 | - | PASSING (runner 12 PASS, Round E-5) |
| NG-JOB-DEV-002 | Job | 비전공 + 개발 직무 + 구현 프로젝트 있음 | P0 | FIXTURED | NON_MAJOR_WITH_IMPLEMENTATION_PROJECT_FOR_DEV_DATA 발화 계약. responsibilityScope.lead+scoreReason+liftOrLimit (3슬롯). D-1 P1b: lead "구현·분석 프로젝트는 전공보다 더 직접적인..." shouldMention regression lock 확정. | 2026-04-30 | - | PASSING (runner 12 PASS, Round D-1 P3) |
